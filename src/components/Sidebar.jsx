...
export function Sidebar({ navigate, page, registrationStep, onLogout, userData, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      {isDrawer && isOpen && (
        <div
          onClick={onClose}
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 48, backdropFilter: 'blur(2px)' }}
        />
      )}
      ...
      <div style={{
        ...
        boxShadow: '4px 0 24px rgba(44,28,18,0.18)',
        transform: isDrawer ? (isOpen ? 'translateX(0)' : 'translateX(-260px)') : 'translateX(0)',
        ...
        <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {isDrawer && (
            <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: 8, width: 30, height: 30, cursor: 'pointer', color: 'rgba(255,255,255,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ padding: '12px 16px 20px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: T.sans, fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{regNo}</div>
            </div>
            <button onClick={onLogout} title="Keluar" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.35)', display: 'flex', padding: 4, borderRadius: 6 }}>
