// desktop-pages-7-8.jsx — Pembayaran + Status & Pengumuman (desktop)

const { C, T, Btn, Card, Badge, Divider, SectionTitle, StepTracker, PageHeader } = window;

// ── PAGE 7: PEMBAYARAN ────────────────────────────────────────
function PagePayment({ navigate, paymentMethod, setPaymentMethod, onStepComplete }) {
  const [confirmed, setConfirmed] = React.useState(false);
  const [loading, setLoading] = React.useState(false);
  const [countdown, setCountdown] = React.useState(23 * 3600 + 45 * 60 + 30);
  ...
  const methods = [
    { id: 'va', label: 'Virtual Account', banks: ['BCA','BNI','BRI','Mandiri'], desc: 'Nomor VA otomatis. Transfer persis sesuai nominal untuk konfirmasi otomatis.', instructions: ['Pilih bank yang kamu gunakan di bawah','Catat atau salin nomor Virtual Account','Transfer persis Rp 250.000 ke nomor VA tersebut','Konfirmasi pembayaran di halaman ini'] },
    { id: 'transfer', label: 'Transfer Bank', banks: ['BCA','BNI','BRI','Mandiri','CIMB'], desc: 'Transfer langsung ke rekening resmi universitas.', instructions: ['Catat nomor rekening: BNI 0123456789 a/n Universitas Nusantara','Transfer persis Rp 250.000','Simpan bukti transfer','Konfirmasi pembayaran di halaman ini'] },
    { id: 'qris', label: 'QRIS', banks: [], desc: 'Scan QR code dengan GoPay, OVO, Dana, ShopeePay, atau m-Banking.', instructions: [] },
  ];
    ...
    return (
      <div style={{ padding: '36px 40px', maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
        <div style={{ background: C.card, borderRadius: 24, padding: '56px 40px', boxShadow: '0 4px 24px rgba(44,44,44,0.1)', border: `1px solid ${C.border}` }}>
          ...
          <p style={{ fontFamily: T.sans, fontSize: 15, color: C.muted, lineHeight: 1.7, marginBottom: 32 }}>Terima kasih! Pendaftaranmu sedang diproses. Kami akan mengirim notifikasi setiap ada perkembangan.</p>
          <div style={{ background: C.bg, borderRadius: 14, padding: '20px 24px', marginBottom: 28, textAlign: 'left' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {[['No. Transaksi', 'TRX-2026-0058219'],['Metode', selected?.label || '-'],['Nominal', 'Rp 250.000'],['Waktu', '22 Apr 2026, 11.32']].map(([k,v]) => (
                <div key={k}><div style={{ fontFamily: T.sans, fontSize: 12, color: C.muted, marginBottom: 3 }}>{k}</div><div style={{ fontFamily: T.sans, fontSize: 14, fontWeight: 600, color: C.text }}>{v}</div></div>
              ))}
            </div>
          </div>
  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: 1100, margin: '0 auto' }}>
      <PageHeader title="Pembayaran Pendaftaran" subtitle="Selesaikan pembayaran untuk melanjutkan proses pendaftaranmu" breadcrumb="Dashboard / Pembayaran"
        actions={
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 28 }}>
        ...
        <div>
                ...
                <div key={m.id} onClick={() => setPaymentMethod(m.id)} style={{ background: C.card, borderRadius: 14, border: sel ? `2px solid ${C.primary}` : `1px solid ${C.border}`, boxShadow: sel ? `0 0 0 3px ${C.primaryLight}` : '0 2px 8px rgba(44,44,44,0.05)', cursor: 'pointer', transition: 'all 0.15s', overflow: 'hidden' }}>
                  <div style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {m.banks.length > 0 && <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>{m.banks.map(b => <span key={b} style={{ fontFamily: T.sans, fontSize: 11, fontWeight: 600, background: C.bg, color: C.textSub, padding: '3px 8px', borderRadius: 6 }}>{b}</span>)}</div>}
                    </div>
                      ...
                      {m.id === 'qris' ? (
                        <div style={{ padding: '20px', display: 'flex', gap: 24, alignItems: 'center' }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontFamily: T.sans, fontSize: 13, color: C.textSub, lineHeight: 1.7 }}>1. Buka aplikasi e-wallet atau m-Banking<br/>2. Pilih fitur Scan QR / QRIS<br/>3. Arahkan kamera ke QR code di sebelah kiri<br/>4. Masukkan nominal Rp 250.000<br/>5. Konfirmasi pembayaran</div>
                          </div>
                        </div>
                      ) : (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Btn size="lg" disabled={!paymentMethod || loading} onClick={handleConfirm}>{loading ? 'Memproses...' : 'Konfirmasi Pembayaran →'}</Btn>
          </div>
        </div>
...
function PageStatus({ navigate, registrationStep }) {
  const steps = [
  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {/* Horizontal step tracker — full width */}
              ...
              return (
                <div key={i} style={{ background: bgColor, borderRadius: 12, padding: '16px 20px', border: `1px solid ${status === 'current' ? C.primary + '40' : status === 'done' ? C.success + '30' : C.border}`, display: 'flex', gap: 16, alignItems: 'center' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: dotColor, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: status !== 'pending' ? `0 3px 10px ${dotColor}50` : 'none' }}>
                    {status === 'done' ? <svg width="14" height="11" viewBox="0 0 14 11" fill="none"><path d="M1.5 5.5l4 4L12.5 1" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      : status === 'current' ? <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#fff' }} />
